...
| MCP クライアント | stdio MCP に対応したもの（Claude Code で検証。Cursor は同梱の `.cursor/mcp.json`、Codex は同じ stdio コマンドで登録） |
...
`npm install` で有効になるもの: `.mcp.json`（Claude Code へ MCP 自動接続）/ `.cursor/mcp.json`（Cursor へ同じサーバーを自動接続。作業指示は `AGENTS.md` を読ませ、`.cursor/rules/melta-ui.mdc` は所在ポインタだけを置く）/ `.claude/settings.json` の PostToolUse hook / lint CLI。
