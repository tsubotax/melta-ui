...
Layer 2 — Specification (machine-readable SSOT)
  design/contracts/
    ...
    ├── rules.json         104 prohibition rules (id + severity + detector)
...
| **contracts/** | JSON | AI + harness | Strict spec: 40 contracts (28 web) + 104 rules + 101 tokens |
...
3. **104 prohibition rules** with explicit `detector` + `alternative`, so an AI that guesses wrong is caught — by CI, by the MCP `check_rule` tool, and by the Claude Code hook.
...
| Route | Count | What |
|-------|-------|------|
| Static auto-detection | **46 / 104** | class-match 34 (same path as MCP `check_rule`) + html-attr 7 + composition 5 (nesting + a11y DOM) |
